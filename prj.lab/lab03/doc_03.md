...
autoContrastColor - функция для автоконтрастирования цветного изображения. Она может выполнять как поканальное с использованием функции autoContrastSingleChannel, так и совместное контрастирование в зависимости от переданного параметра.
Совместное контрастирование выполняется для всех каналов одновременно. Для этого используется следующая формула:

$$ f(c) = c_{min} + (c - c_{low}) * \frac{c_{max}-c_{min}}{c_{high} - c_{low}} $$

cmin, cmax - новый эффективный диапазон,
clow, chigh - старый эффективный диапазон.

Описание Алгоритма
- Конвертация изображения: Если глубина изображения не 8-битная, оно конвертируется в 8-битную глубину.
- Создание объединенной гистограммы: Изображение разделяется на каналы (BGR), и для каждого канала вычисляется гистограмма. Эти гистограммы складываются для получения объединенной гистограммы.
- Определение старых порогов (clow и chigh): На основании объединенной гистограммы определяются пороги черного и белого (clow и chigh) по квантилям.
- Установка новых порогов (cmin и cmax): Новые пороги устанавливаются как 0 и 255 соответственно.
- Применение автоконтрастирования: К каждому пикселю изображения применяется формула для автоконтрастирования с использованием вычисленных порогов.
